...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        var query = [ { $match: { category: { $ne: null } } },
        ...
        this.db.collection('items').aggregate(query).toArray(function(err, categories){
            ...
            assert.equal(null, err);
                ...
                category.num = category.num + doc.num;        
            });
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
        ...
        if(category === 'All')
        {
        var getQuery = [     { $sort: { _id: 1}},
                             { $skip: itemsToSkip },
                             { $limit: itemsPerPage } ];
                
        } else {
        var getQuery = [ { $match: { category: category}},
                         { $sort: { _id: 1}},
                         { $skip: itemsToSkip },
                         { $limit: itemsPerPage } ];
        }
        ...
        var pageItems = [];
        for (var i=0; i<5; i++) {
            pageItems.push(pageItem[i]);
            }
        
        this.db.collection('items').aggregate(getQuery).
    ...
    this.getNumItems = function(category, callback) {
        ...
        if(category === 'All')
        {
            ...
            var numQuery = { 'category': category };
        }          
               
        this.db.collection('items').find(numQuery).count(function(err, count){
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
        ...
        var searchQuery = [ { $match: {$text: { $search: query } } }, 
                            { $project: { _id: 1, title: 1, 
                              img_url: 1, description: 1, 
                              slogan: 1, score: { $meta: 'textScore' } } }, 
                            //{ $match: { score: { $gt: 1.0 } } },
        ...
        this.db.collection('items').aggregate(searchQuery).toArray(function(err, items){
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        */
        var numSearchQuery = { $text: { $search: query } };

        this.db.collection('items').find(numSearchQuery).count(function(err, count){
    ...
    this.getItem = function(itemId, callback) {
         ...
         *
         */
        var itemQuery = { _id: itemId };
        var item = this.createDummyItem();

        this.db.collection('items').find(itemQuery).toArray(function(err, result){
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
        ...
        /*var myReviewQuery = [ { _id: itemId }, 
                             { $push: { reviews: reviewDoc }} ];*/

        var doc = this.createDummyItem();
        doc.reviews = [reviewDoc];

        this.db.collection('items').update({ _id: itemId }, 
